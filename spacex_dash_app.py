...
def get_pie_chart(entered_site, entered_pl):
    filtered_df = spacex_df
    filtered_df['Count'] = 1
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df[
            (filtered_df['Payload Mass (kg)'] >= entered_pl[0]) & (filtered_df['Payload Mass (kg)'] <= entered_pl[1])], 
        x='Payload Mass (kg)', 
        y='class',
        ...
        fig = px.scatter(filtered_df[
            (filtered_df['Payload Mass (kg)'] >= entered_pl[0]) 
            & (filtered_df['Payload Mass (kg)'] <= entered_pl[1])
